...
#Importing of data
df=pd.read_csv("py2015.csv")
print(df.head())

#Removing of extreme outlier data
df=df[df['RH']<=100];df=df[df['WindSpeed.mph.']<=100];df=df[df['Ozone']>=0]
...
data= SWIC[['Ozone','Temp','Pressure','RH','WindSpeed.mph.','WindDir','Rainfall','VPD']]
data.info()
...
for n in range(10000):
    X_train,X_test,y_train,y_test= sklearn.model_selection.train_test_split(X,y,test_size=.15)
    linear=linear_model.LinearRegression()
    linear.fit(X_train,y_train)
    accuracy.append(linear.score(X_test,y_test))
    if linear.score(X_test,y_test)==max(accuracy):
        best_regression=linear
...
print(mean(accuracy))
print(best_regression.score(best_X_test,best_y_test))
...
b_predictions = best_regression.predict(best_X_test)
scatter(best_y_test,b_predictions)
scatter(y_test,predictions)
for x in range(len(predictions)):
    print(format(predictions[x],',.2f'),'\t',format(y_test[x],',.2f'))

#Applying regression onto actual data from SWIC missing in the Planetarium data
MSD=pd.read_csv("Missing SWIC Dates 2015.csv")
missing_dates= array(MSD.pop('Rdate'))
MSD.info()
SWIC_info=array(MSD)
missing_ozone=best_regression.predict(SWIC_info)
file_contents=[missing_dates,missing_ozone]
file_contents=pd.DataFrame(file_contents)
file_contents=file_contents.transpose()
file_contents.to_csv("Missing Planetarium Ozone 2015.csv")
######################################################################
...
dataset=pd.read_csv("py2015colnodate.csv")
...
sns.pairplot(train_dataset[["Ozone", "OzoneG", "RH","RHG","Temp","TempG"]], diag_kind="kde")
show()
